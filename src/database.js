...
Database.prototype.getRandomQuote = function(callback) {
	this.pool.query("SELECT quote, author, submitted FROM Quotes, (SELECT RAND() * (SELECT MAX(id) FROM Quotes) AS tid) AS Tmp WHERE Quotes.id >= Tmp.tid ORDER BY id ASC LIMIT 1",
		function(err, rows) {
			if(err) {
				if(callback) { callback(err); }
				else throw err;
